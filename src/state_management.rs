...
pub trait ThreadState{
    // this allow is here and a bunch of places below to ensure certain gauruntees about the abi
    // that can be leveraged to allow for different methods of restoration
    #[allow(improper_ctypes_definitions)]
    unsafe extern "C" fn restore(self);
}
pub struct FuncResume<T,F:FnOnce(T)>{
    state: T,
    call: F
}
    ...
    extern "C" fn new()-> RegisterReset{
        ...
        unsafe {
            asm!("mov {}, rax",
                 ...
                 out(reg) r14,
                 out(reg) r15);
            // TODO: set rsi/rpi to the values they'll be upon leaving this function
